...
# Age (days) of a local ODC DB, or 9999 if none
odc_cache_age_days() {
  local db=""
  [[ -f "$ODC_DATA_DIR/odc.mv.db" ]] && db="$ODC_DATA_DIR/odc.mv.db"
  [[ -z "$db" && -f "$ODC_DATA_DIR/dc.h2.db" ]] && db="$ODC_DATA_DIR/dc.h2.db"
  if [[ -z "$db" ]]; then
    echo 9999; return 0
  fi
  local now mtime
...
# Try downloading a prebuilt cache tarball when missing
bootstrap_odc_cache_from_url() {
  local url="${ODC_CACHE_URL:-}"
  [[ -n "$url" ]] || return 1
  echo "⬇️  ODC cache missing — downloading prebuilt cache from ODC_CACHE_URL…"
  mkdir -p "$ODC_DATA_DIR"
  local tmp="${SEC_REPORT_DIR}/odc-cache.tgz"
  (curl -fsSL "$url" -o "$tmp" || wget -qO "$tmp" "$url") || return 1
  tar -xzf "$tmp" -C "$ODC_DATA_DIR" || return 1
  echo "✅ ODC cache restored to $ODC_DATA_DIR"
  return 0
}
...
run_odc() {
  ...
  mkdir -p "$SEC_REPORT_DIR" "$ODC_DATA_DIR"

  # Policy knobs (sensible defaults)
  local MODE="${ODC_ALLOW_UPDATE:-auto}"          # auto|true|false
  local MAX_AGE="${ODC_MAX_CACHE_AGE_DAYS:-14}"
  local HAVE_KEY="false"
  [[ -n "${NVD_API_KEY:-}" ]] && HAVE_KEY="true"

  # Do we have a cache right now?
  local CACHE_PRESENT="false"
  [[ -f "$ODC_DATA_DIR/odc.mv.db" || -f "$ODC_DATA_DIR/dc.h2.db" ]] && CACHE_PRESENT="true"

  # If cache is missing, first try to restore from URL, else we may update once (if key)
  if [[ "$CACHE_PRESENT" == "false" ]]; then
    if bootstrap_odc_cache_from_url; then
      CACHE_PRESENT="true"
    fi
  fi
  ...
  if [[ "$CACHE_PRESENT" == "true" ]]; then
    # Decide whether to update a present cache
    local AGE_DAYS; AGE_DAYS="$(odc_cache_age_days)"
    case "$MODE" in
      true)
        if [[ "$HAVE_KEY" == "true" ]]; then
          echo "ℹ️ ODC: updates enabled (explicit). Cache age=${AGE_DAYS}d."
          UPDATE_ARGS=()
        else
          echo "⚠️ ODC: updates requested but NVD_API_KEY is empty → using --noupdate. Cache age=${AGE_DAYS}d."
          UPDATE_ARGS=( --noupdate )
        fi
        ;;
      auto)
        if (( AGE_DAYS >= MAX_AGE )) && [[ "$HAVE_KEY" == "true" ]]; then
          echo "ℹ️ ODC: cache age=${AGE_DAYS}d (>= ${MAX_AGE}) → allowing one update."
          UPDATE_ARGS=()
        else
          echo "ℹ️ ODC: cache age=${AGE_DAYS}d (< ${MAX_AGE} or no key) → using --noupdate."
          UPDATE_ARGS=( --noupdate )
        fi
        ;;
      *)
        echo "ℹ️ ODC: updates disabled (ODC_ALLOW_UPDATE=$MODE) → using --noupdate."
        UPDATE_ARGS=( --noupdate )
        ;;
    esac
  else
    # Still no cache
    if [[ "$HAVE_KEY" == "true" ]]; then
      echo "ℹ️ ODC: no cache found; NVD_API_KEY present → performing one update to seed DB."
      UPDATE_ARGS=()  # allow update
    else
      # Graceful skip with strong guidance
      note_result "$ALLURE_RESULTS_DIR" "Dependency-Check: Skipped" \
        "No local cache at $ODC_DATA_DIR and no NVD_API_KEY provided.\n\
Set either:\n\
  • NVD_API_KEY=<your key> (ODC will update once and seed the cache), or\n\
  • ODC_CACHE_URL=<tarball url> (script will download a prebuilt cache).\n\
You may also disable this tool by removing 'odc' from --tools."
      return 0
    fi
  fi
  ...
  if [[ -s "$REPORT_JSON" ]]; then
    ...
    note_result "$ALLURE_RESULTS_DIR" "Dependency-Check: No JSON" "Tool produced no output."
  fi
}
